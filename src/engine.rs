use rustinolla::{EngineEvent, Render};
        ...
        match self.event_pump.wait_event() {
            ...
            Event::MouseButtonDown {
                ...
                if mouse_btn == MouseButton::Left {
                    if x < WIDTH as i32 / 3 {
                        x = 0
                    } else if x < WIDTH as i32 / 3 * 2 {
                        x = 1
                    } else {
                        x = 2
                    }

                    if y < HEIGHT as i32 / 3 {
                        y = 0
                    } else if y < HEIGHT as i32 / 3 * 2 {
                        y = 1
                    } else {
                        y = 2
                    }
...
impl Engine {
    ...
    pub fn draw_backround(&mut self) -> Result<(), String> {
        ...
        for i in 1..=2 {
            self.canvas
                .fill_rect(Rect::new((i * (WIDTH as i32 / 3)) - 5, 0, 10, HEIGHT))?;
            self.canvas
                .fill_rect(Rect::new(0, (i * (HEIGHT as i32 / 3)) - 5, WIDTH, 10))?;
        }
