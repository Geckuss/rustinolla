use rustinolla::{Player, Render};
use rustinolla::{BLUE, HEIGHT, RED, WIDTH, YELLOW};
use sdl2::rect::Rect;
...
impl Render for GameState {
    fn render(&self, canvas: &mut Canvas<Window>) -> Result<(), String> {
        ...
        if let Some((_, start, end)) = self.has_a_winner() {
            draw_bar(start, end, canvas)?;
        }
...
impl GameState {
    ...
    pub fn is_full(&self) -> bool {
        self.grid.iter().filter(|row| row.contains(&None)).count() == 0
    }
}
pub fn draw_bar(
    start: (i32, i32),
    ...
    match orientation {
        BarOrientation::Horizontal => {
            ...
            let rect = Rect::new(x as i32, y as i32, WIDTH - padding as u32 * 2, HEIGHT / 100);
            canvas.fill_rect(rect)?;
        }
        BarOrientation::Vertical => {
            let x = WIDTH / 3 * sx as u32 + HEIGHT / 6;
            let y = HEIGHT / 3 * sy as u32 + padding as u32;
            let rect = Rect::new(x as i32, y as i32, WIDTH / 100, HEIGHT - padding as u32 * 2);
            canvas.fill_rect(rect)?;
        }
...
#[allow(unused)]
fn get_bar_orientation(start: (i32, i32), end: (i32, i32)) -> BarOrientation {
    use BarOrientation::*;
    match (start, end) {
